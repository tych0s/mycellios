...
export interface DesktopSettings {
  ...
  /**
   * Signed runtime/product components use their own lightweight channel. The
   * The native service bootstrap remains on its platform package updater.
   */
...
export interface DesktopDeveloperChannelEnrollment {
  feedUrl: string;
  keyId: string;
...
export interface ChatResponse {
  ...
  trustPolicy?: "default" | "trusted-only";
  boundaryPolicy?: "trusted-edges" | "pinned-edges";
  pinnedIdentityCount?: number;
...
export interface SystemLogEntry {
  ...
  source: "node" | "coordinator" | "worker" | "runtime" | "control-ui" | "network";
  event: string;
  message: string;
  details?: string;
}
