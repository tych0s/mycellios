import type { ChatResponse, ChatStreamUpdate } from "../contracts/control-api.js";
...
interface MycelliosStreamChunk {
  ...
  x_network?: {
    ...
    recovery_mode?: "none" | "prompt-replay" | "deterministic-prefix-replay";
    recovery_attempts?: number;
    replayed_token_events?: number;
    execution_receipt_id?: string | null;
    trust_policy?: "default" | "trusted-only";
    boundary_policy?: "trusted-edges" | "pinned-edges";
    pinned_identity_count?: number;
  };
...
export async function consumeChatCompletionStream(
  ...
  let recoveryMode: ChatResponse["recoveryMode"];
  let recoveryAttempts: number | undefined;
  let replayedTokenEvents: number | undefined;
  let executionReceiptId: string | undefined;
  let trustPolicy: ChatResponse["trustPolicy"];
  let boundaryPolicy: ChatResponse["boundaryPolicy"];
  let pinnedIdentityCount: number | undefined;
  ...
  const emit = (
    ...
    extra: Partial<Pick<
      ...
      "recoveryMode" | "recoveryAttempts" | "replayedTokenEvents"
      | "executionReceiptId"
      | "trustPolicy" | "boundaryPolicy" | "pinnedIdentityCount"
    >> = {},
  ) => onUpdate({
    requestId,
    model,
  ...
  const consumeEvent = (event: string) => {
    ...
    if (chunk.x_network?.recovery_mode !== undefined) recoveryMode = chunk.x_network.recovery_mode;
    if (chunk.x_network?.recovery_attempts !== undefined) recoveryAttempts = chunk.x_network.recovery_attempts;
    if (chunk.x_network?.replayed_token_events !== undefined) replayedTokenEvents = chunk.x_network.replayed_token_events;
    if (chunk.x_network?.execution_receipt_id) executionReceiptId = chunk.x_network.execution_receipt_id;
    if (chunk.x_network?.trust_policy) trustPolicy = chunk.x_network.trust_policy;
    if (chunk.x_network?.boundary_policy) boundaryPolicy = chunk.x_network.boundary_policy;
    if (chunk.x_network?.pinned_identity_count !== undefined) pinnedIdentityCount = chunk.x_network.pinned_identity_count;
    const recovery = {
      ...(recoveryMode === undefined || recoveryMode === "none" ? {} : { recoveryMode }),
      ...(recoveryAttempts === undefined ? {} : { recoveryAttempts }),
      ...(replayedTokenEvents === undefined ? {} : { replayedTokenEvents }),
      ...(executionReceiptId === undefined ? {} : { executionReceiptId }),
      ...(trustPolicy === undefined ? {} : { trustPolicy }),
      ...(boundaryPolicy === undefined ? {} : { boundaryPolicy }),
      ...(pinnedIdentityCount === undefined ? {} : { pinnedIdentityCount }),
    };
