...
def scale_cell_fixture_layer(layer: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Use fan-in-scaled matrices for cache/fork functional regression fixtures.

    Unit-variance random matrices compound large attention and MLP activations
    across decode steps. These tests exercise state transitions, not numerical
    conditioning of untrained weights. Preserve random norms, shapes and seeds
    while bounding the projection scale on different CPU kernels.
    """
    return {
        name: value / value.shape[1] ** 0.5 if value.ndim == 2 else value
...
def assert_fp32_cell_close(actual: torch.Tensor, expected: torch.Tensor) -> None:
    ...
    orders. The original unscaled random weights amplified roundoff:
    ...
    Keep a fixed 32*eps budget for the fan-in-scaled 16-wide layers. Compare the worst
