...
_ALLOWED_MY_ARG_FLAGS = {
    "--boundaries", "--codec", "--device", "--first-stage-host",
    "--first-stage-port", "--layer-end", "--layer-start", "--listen-host",
    "--listen-port", "--max-active-sequences", "--max-batch-size", "--model",
    "--nakshatra-compute-api", "--nakshatra-context-tokens",
    "--nakshatra-daemon-bin", "--nakshatra-gpu-layers",
    "--nakshatra-manifest-sha256", "--nakshatra-package",
    "--nakshatra-package-id", "--nakshatra-pipeline-id", "--next-host",
    "--next-layer-end", "--next-port", "--no-speculation-probes",
    "--physical-batch-window-ms", "--port", "--prefill-chunk-tokens",
    "--return-advertise-host", "--return-bind-host", "--return-host",
    "--return-port", "--root-batch-window-ms", "--speculation",
    "--speculative-max-draft-tokens", "--stages", "--threads",
    "--threads-per-stage", "--total-layers",
}


def _validated_my_args(value):
    """Accept only the runtime CLI surface generated by the orchestrator."""
    if not isinstance(value, list) or not value:
        raise ValueError("my_args debe ser una lista no vacía")
    result = []
    for arg in value:
        if not isinstance(arg, str) or not arg or len(arg) > 4096:
            raise ValueError("my_args contiene un argumento inválido")
        if any(ch in arg for ch in ("\x00", "\r", "\n")):
            raise ValueError("my_args contiene caracteres de control")
        if arg.startswith("--") and arg not in _ALLOWED_MY_ARG_FLAGS:
            raise ValueError(f"flag no permitido en my_args: {arg}")
        result.append(arg)
    return result
...
def _start(cfg):
    ...
    with _START_LK:
        ...
        cmd = [sys.executable, "-m", mod] + _validated_my_args(cfg["my_args"])
        # The executable/module are fixed and every dynamic argument is validated
        # against the orchestrator's explicit CLI surface above.
        m = subprocess.Popen(cmd, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)  # codeql[py/command-line-injection]
        _PROCS["mycellios"] = m
