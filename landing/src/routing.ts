...
export const PANEL_ROUTES: readonly PanelRoute[] = [
  ...
  { id: "contribute", label: "This device", width: "standard", modes: ["simple", "developer"], roles: EVERY_ROLE, primaryAction: "Connect device" },
  { id: "join", label: "Join", width: "standard", modes: [], roles: EVERY_ROLE, primaryAction: "Choose connection" },
...
const LEGACY_PANEL_VIEWS: Readonly<Record<string, PanelView>> = { history: "network", tasks: "jobs", chat: "inference" };
...
export function panelViewFromLocation(pathname: string, search: string, mobile = false): PanelView {
  ...
  if (mobile || pathname.startsWith("/mobile")) return "contribute";
...
export function panelLocation(view: PanelView, mobile = false): string {
  if (mobile) return view === "contribute" ? "/mobile/" : `/mobile/?view=${view}`;
