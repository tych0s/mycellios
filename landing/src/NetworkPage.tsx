...
export function historyForRange(history: NetworkTelemetryHistory | null, range: NetworkHistoryRange): NetworkTelemetryHistory | null {
...
type NodeState = "serving" | "standby" | "joining" | "offline";
...
export function NetworkPage({
  ...
  const [historyRange, setHistoryRange] = useState<NetworkHistoryRange>(initialHistory?.range ?? "30d");
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const response = await fetch("/public/v1/snapshot", {
        cache: "no-store",
        signal: AbortSignal.any([controller.signal, AbortSignal.timeout(12_000)]),
      });
      if (!response.ok) throw new Error(`Network snapshot returned HTTP ${response.status}.`);
      const next = await response.json() as NetworkSnapshot;
      if (controller.signal.aborted) return;
      setSnapshot(next);
      setError(null);
    } catch (caught) {
      if (!controller.signal.aborted) setError(caught instanceof Error && caught.name === "TimeoutError"
        ? "The network snapshot timed out. Try again." : caught instanceof Error ? caught.message : "The network snapshot is unavailable.");
    } finally {
  ...
  const refreshHistory = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/public/v1/history?range=${historyRange}`, {
        cache: "no-store",
        signal: AbortSignal.any([controller.signal, AbortSignal.timeout(12_000)]),
      });
      ...
      if (!controller.signal.aborted) setHistoryError(caught instanceof Error && caught.name === "TimeoutError"
        ? "The network history timed out. Try again." : caught instanceof Error ? caught.message : "The network history is unavailable.");
    } finally {
  ...
  const visibleHistory = historyForRange(history, historyRange);
  function selectHistoryRange(next: NetworkHistoryRange) {
    if (next === historyRange) return;
  ...
  const dailyJobs = useMemo(() => deriveDailyJobs(visibleHistory?.samples ?? []), [visibleHistory]);
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      centerX = width / 2;
      centerY = height / 2 + (viewMode === "globe" ? 20 : 0);
      radius = Math.min(width, height) * (viewMode === "globe" ? 0.4 : 0.44);
    };
    ...
    const shouldAnimate = () => animate && !reduceMotion && document.visibilityState === "visible" && inViewport;
  ...
  useEffect(() => { drawRef.current?.(); }, [nodes, arcs, selectedId]);
  ...
  const observed = (value: string | number) => hasData ? String(value) : "—";
  ...
  const inactive = nodes.filter((node) => node.state !== "serving").slice(0, 3);
    ...
    <div className="net-stage">
      ...
      {viewMode === "overview" && <section className="net-dashboard" aria-label="Live network overview">
        <header className="net-dashboard-head">
          <span className="net-hero-kicker"><i className={error || stale ? "degraded" : ""} />{error ? hasData ? "LAST VERIFIED SNAPSHOT" : "NETWORK UNAVAILABLE" : loading ? "CONNECTING" : stale ? "STALE SNAPSHOT" : "LIVE NETWORK"}</span>
        </header>
        <div className="net-hero-metrics">
          <span><strong>{observed(snapshot.summary.completedJobs.toLocaleString("en-US"))}</strong>completed jobs</span>
          <span><strong>{observed(snapshot.summary.connected)}</strong>connected nodes</span>
          <span><strong>{observed(snapshot.summary.online)}</strong>online now</span>
          <span><strong>{observed(metrics.gpuCount)}</strong>GPUs online</span>
          <span><strong>{observed(metrics.availableModels)}</strong>models serving</span>
          <span><strong>{observed(metrics.activeJobs.length)}</strong>active tasks</span>
          <span><strong>{observed(formatMemory(metrics.memoryGb))}</strong>registered memory</span>
        </div>
        <div className="net-dashboard-visuals">
          <article className="net-globe-card">
            <canvas
              ref={canvasRef}
              className="net-globe"
              aria-label={hasData ? `Interactive globe showing ${nodes.length} announced Mycellios nodes` : "Interactive globe awaiting network snapshot"}
              role="img"
            />
            <header><span>LIVE TOPOLOGY</span><strong>{hasData ? `${snapshot.summary.online} online · ${metrics.regions} regions` : "Awaiting snapshot"}</strong></header>
            <button type="button" onClick={() => selectViewMode("globe")} aria-label="Open full-screen globe"><ArrowRight /></button>
          </article>
          <DashboardChart title="Online capacity" unit="GB" samples={visibleHistory?.samples ?? []} series={[{ key: "offeredVramMb", label: "Online", tone: "bronze", divisor: 1024 }, { key: "freeVramMb", label: "Free", tone: "sage", divisor: 1024 }]} />
          <DashboardChart title="Node availability" unit="nodes" samples={visibleHistory?.samples ?? []} series={[{ key: "connectedNodes", label: "Connected", tone: "ink" }, { key: "onlineNodes", label: "Online", tone: "sage" }]} />
          <DashboardChart title="Model fabric" unit="models" samples={visibleHistory?.samples ?? []} series={[{ key: "activeModels", label: "Models", tone: "ink" }, { key: "modelReplicas", label: "Replicas", tone: "bronze" }]} />
          <DashboardChart title="Inference load" unit="jobs" samples={visibleHistory?.samples ?? []} series={[{ key: "inflightJobs", label: "In flight", tone: "bronze" }, { key: "runningJobs", label: "Running", tone: "sage" }]} />
          <DashboardChart title="Completed work" unit="jobs" samples={visibleHistory?.samples ?? []} series={[{ key: "completedJobs", label: "Completed", tone: "sage" }]} />
        </div>
      </section>}

      {viewMode === "overview" && <a className="net-hud net-scroll-cue" href="#network-evidence"><span>SCROLL</span>Historical telemetry · node ledger · methodology <b aria-hidden="true">↓</b></a>}
      ...
      <div className="net-hud net-legend" role="list" aria-label="Node state legend">
        ...
        <span role="listitem"><i style={{ background: "rgba(255,255,255,0.16)" }} />offline</span>
      </div>

      <div className="net-hud net-stats net-stats-left" aria-label="Network capacity">
        <Stat value={observed(metrics.gpuCount)} label="GPUS ONLINE" />
        <Stat value={observed(metrics.regions)} label="REGIONS" />
        <Stat value={observed(metrics.availableModels)} label="MODELS SERVING" />
      </div>
      ...
      >
        <Stat value={observed(metrics.activeJobs.length)} label="ACTIVE TASKS" />
        <Stat value={observed(metrics.tokensServed.toLocaleString("en-US"))} label="TOKENS IN RECENT JOBS" />
        <Stat value={observed(formatMemory(metrics.memoryGb))} unit label="REGISTERED GPU MEMORY" />
      </div>

      <div className="net-hud net-ticker">
        <div className="net-ticker-label">{inactive.length > 0 ? "nodes not serving" : "node activity"}</div>
        {inactive.length === 0
          ? <div className="net-ticker-row empty">
              {!hasData ? "Waiting for network snapshot." : nodes.length === 0 ? "Waiting for the first node." : "Every announced node is serving."}
            </div>
          : inactive.map((node, index) => <div
              key={node.worker.id}
            ...
            >
              ...
              {` ${node.state === "standby" ? "on standby" : node.state} · `}
              <span className="loc">
      ...
      <div className="net-hud net-cmd">
        <code><span>{SNAPSHOT_COMMAND}</span></code>
      ...
      {!loading && !error && nodes.length === 0 && <div className="net-empty">
    ...
    <section className="net-data" id="network-evidence" aria-labelledby="net-data-title">
      <div className="net-data-shell">
        ...
        <div className="net-proof-strip" aria-label="Evidence provenance">
          <span><i className="physical" />Latest snapshot <strong>{hasData ? relativeTime(snapshot.capturedAt) : loading ? "connecting" : "unavailable"}</strong></span>
          <span><i className="stored" />Persisted history <strong>{visibleHistory ? `every ${visibleHistory.intervalMinutes} min` : "awaiting samples"}</strong></span>
          ...
          <div className="net-signal-groups">
            <article className="net-signal-group">
              ...
              <div className="net-signal-cards">
                <SignalCard label="Workers online" value={observed(metrics.activeWorkers.length)} tone="healthy" />
                <SignalCard label="Native nodes" value={observed(metrics.activeWorkers.filter((worker) => worker.kind !== "browser").length)} />
                <SignalCard label="Browser nodes" value={observed(snapshot.summary.mobile)} />
                <SignalCard label="Models serving" value={observed(metrics.availableModels)} />
                <SignalCard label="Tasks in flight" value={observed(metrics.activeJobs.length)} />
                <SignalCard label="Registered memory" value={observed(formatMemory(metrics.memoryGb))} />
              </div>
            </article>
            <article className="net-signal-group">
              <header><strong>NETWORK</strong><span>persisted telemetry</span></header>
              <div className="net-signal-cards">
                <SignalCard label="Completed jobs" value={observed(snapshot.summary.completedJobs.toLocaleString("en-US"))} />
                <SignalCard label="Samples stored" value={visibleHistory?.samples.length.toLocaleString("en-US") ?? "—"} />
                <SignalCard label="Retention" value={visibleHistory ? `${visibleHistory.retentionDays}d` : "—"} />
                <SignalCard label="Sampling" value={visibleHistory ? `${visibleHistory.intervalMinutes}m` : "—"} />
              </div>
            </article>
        ...
        <section className="net-wall" aria-labelledby="net-wall-network-title">
          <header className="net-wall-head">
            ...
            <div className="net-range" role="group" aria-label="History range">
              {HISTORY_RANGES.map(({ value, label }) => <button type="button" key={value} className={historyRange === value ? "active" : ""} aria-pressed={historyRange === value} onClick={() => selectHistoryRange(value)}>{label}</button>)}
            </div>
          </header>
          ...
          {historyLoading && !visibleHistory && <div className="net-history-loading" role="status"><span />Reading persisted samples…</div>}
          {visibleHistory && visibleHistory.samples.length > 0 ? <div className="net-wall-grid">
            <StripedBarChart
              full
            ...
            />
            <StepAreaChart
              title="online capacity (GB)"
              note="observed VRAM on connected online workers ÷ 1024"
            ...
            <StepLineChart
              ...
              note={`sampled every ${visibleHistory.intervalMinutes} min · ${visibleHistory.samples.length} persisted observations`}
              samples={visibleHistory.samples}
              metric="onlineNodes"
              unit="nodes"
              tone="ivory"
        ...
        <section className="net-wall" aria-labelledby="net-wall-snapshot-title">
          ...
          <div className="net-wall-stats">
            <WallStat value={observed(snapshot.summary.registered.toLocaleString("en-US"))} label="registered nodes" />
            <WallStat value={observed(snapshot.summary.connected.toLocaleString("en-US"))} label="connected" />
            <WallStat value={observed(snapshot.summary.online.toLocaleString("en-US"))} label="online now" />
            <WallStat value={observed(metrics.gpuCount)} label="GPUs online" />
            <WallStat value={observed(metrics.availableModels)} label="models serving" />
            <WallStat value={observed(snapshot.summary.completedJobs.toLocaleString("en-US"))} label="completed jobs" />
          </div>
        </section>
        ...
        <section className="net-wall" aria-labelledby="net-wall-activity-title">
          <header className="net-wall-head">
            <h2 id="net-wall-activity-title">recorded activity</h2>
            <span className="net-wall-note">latest stored jobs · age shown</span>
          </header>
        ...
        <section className="net-ledger" aria-labelledby="net-ledger-title">
          ...
          <div className="net-ledger-table" role="table" aria-label="Public node ledger">
            ...
            {metrics.activeWorkers.length === 0 && <div className="net-ledger-empty">{hasData ? "No online nodes are present in the current snapshot." : "Awaiting network snapshot."}</div>}
          </div>
        </section>
 ...
 */
function WallChartFrame({ count, max, banded = false, compact = false, xLabels, tooltip, ariaLabel, children }: {
  count: number;
  max: number;
  banded?: boolean;
  compact?: boolean;
  ...
  useEffect(() => {
    ...
    if (!svg) return;
    const measure = () => {
      const nextWidth = Math.round(svg.getBoundingClientRect().width);
      if (nextWidth > 0) setObservedWidth(nextWidth);
    };
    measure();
  ...
  const baseWidth = compact ? 480 : 720;
  const baseHeight = compact ? 190 : 220;
  const width = observedWidth ?? baseWidth;
  const height = observedWidth === null ? baseHeight : Math.max(compact ? 150 : 180, Math.round(width * baseHeight / baseWidth));
  const left = 46;
  const right = 14;
  const top = 16;
  const bottom = 30;
  ...
  const tipWidth = tip
    ? Math.min(width - 8, Math.max(104, Math.max(tip.title.length, ...tip.lines.map((line) => `${line.label} ${line.value}`.length)) * 7.6 + 22))
    : 0;
  const tipLineHeight = 18;
  const tipHeight = tip ? 24 + tip.lines.length * tipLineHeight : 0;
  const tipX = tip && hover !== null
    ? Math.max(4, Math.min(width - tipWidth - 4, x(hover) + 12 + tipWidth > width - right ? x(hover) - 12 - tipWidth : x(hover) + 12))
    : 0;

  return <svg
    ref={svgRef}
    className={`net-wall-chart${compact ? " compact" : ""}`}
  ...
  >
    ...
    <text className="axis" x={left} y={height - 8}>{width < 500 ? xLabels[0].split(",")[0] : xLabels[0]}</text>
    <text className="axis" x={left + plotWidth / 2} y={height - 8} textAnchor="middle">{width < 500 ? xLabels[1].split(",")[0] : xLabels[1]}</text>
    <text className="axis" x={width - right} y={height - 8} textAnchor="end">{width < 500 ? xLabels[2].split(",")[0] : xLabels[2]}</text>
    {tip && hover !== null && <g className="net-wall-tip" pointerEvents="none">
      ...
      {tip.lines.map((line, index) => <text key={line.label} className={`tip-line ${line.tone ?? ""}`} x={tipX + 11} y={top + 4 + 30 + index * tipLineHeight}>{line.label} {line.value}</text>)}
    </g>}
  </svg>;
}
...
function sampleXLabels(points: NetworkTelemetrySample[], compact = false): [string, string, string] {
  const span = Date.parse(points.at(-1)!.capturedAt) - Date.parse(points[0]!.capturedAt);
  const label = (sample: NetworkTelemetrySample) => span <= 36 * 60 * 60 * 1000
    ? formatChartShortTimestamp(sample.capturedAt)
    : compact ? formatChartShortDate(sample.capturedAt) : formatChartDate(sample.capturedAt);
  return [
    label(points[0]!),
    label(points[Math.floor((points.length - 1) / 2)]!),
    label(points.at(-1)!),
  ];
}
...
function StripedBarChart({ title, note, full = false, buckets }: { title: string; note?: string; full?: boolean; buckets: DailyJobBucket[] }) {
  ...
  return <WallPanel title={title} note={note} full={full}>
    {buckets.length > 0 ? <WallChartFrame
      ...
      xLabels={[
        buckets.length > 1 ? buckets[0]!.day.slice(5) : "",
        buckets.length > 2 ? buckets[Math.floor((buckets.length - 1) / 2)]!.day.slice(5) : buckets.length === 1 ? buckets[0]!.day.slice(5) : "",
        buckets.length > 1 ? buckets.at(-1)!.day.slice(5) : "",
      ]}
      tooltip={(index) => ({
...
function StepAreaChart({ title, note, full = false, samples, metric, divisor = 1, unit, tone }: {
  ...
  return <WallPanel title={title} note={note} full={full}>
    <WallChartFrame
      ...
      xLabels={sampleXLabels(points, !full)}
      tooltip={(index) => ({
...
function formatChartShortDate(value: string): string {
  const parsed = Date.parse(value);
  return Number.isFinite(parsed) ? new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(parsed) : "—";
}

function formatChartShortTimestamp(value: string): string {
  const parsed = Date.parse(value);
  if (!Number.isFinite(parsed)) return "—";
  const date = new Intl.DateTimeFormat("en", { day: "numeric" }).format(parsed);
  const time = new Intl.DateTimeFormat("en", { hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(parsed);
  return `${date} ${time}`;
}
