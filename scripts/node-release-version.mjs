#!/usr/bin/env node
/**
 * Mantiene la version del producto nativo POR DELANTE de todo lo ya publicado.
 *
 * POR QUE EXISTE. La version de release sale de `package.json` y de ningun otro
 * sitio: `node-build.yml` la lee, exige que `package-lock.json` coincida, y
...
function bump() {
  ...
  console.log(`Native node release version ${declared} -> ${next}.`);
}
...
function check() {
  ...
  if (published === null) {
    console.log(
      `Native node release version ${declared}; no git tags readable, nothing to compare.`,
    );
    return;
  }
  ...
  if (taken.length > 0) {
    ...
    throw new Error(
      `Native node release version ${declared} is not ahead of published tag v${highest}.\n`
  ...
  console.log(
    `Native node release version ${declared} is ahead of all `
